...
$(document).ready(function () {
    if (
        document.cookie.indexOf("mm8_stock_limit_status=accepted") !== -1 ||
        document.cookie.indexOf("mm8_stock_limit_status=reject") !== -1
    ) {
...
$(document).on("click", ".product-stock-alert-hide", function () {
    document.cookie =
        "mm8_stock_limit_status=accepted; max-age=" +
        60 * 60 * 24 * 30 +
        "; path=/";
    $(".product-limited-stock-alert").hide();
});
...
$(document).on("click", ".product-stock-limit-close", function () {
    document.cookie =
        "mm8_stock_limit_status=reject; max-age=" + 60 * 20 + "; path=/";
    $(".product-limited-stock-alert").hide();
});
        ...
        success: function (response) {
            ...
            if (
                document.cookie.indexOf(
                    "mm8_restock_request_status=accepted"
                ) !== -1 ||
                document.cookie.indexOf(
                    "mm8_restock_request_status=reject"
                ) !== -1
            ) {
...
$(document).on("click", ".product-restock-request-alert-hide", function () {
    document.cookie = "mm8_restock_request_status=accepted; path=/";
    $(".product-restock-stock-alert").hide();
});
